...
def bruteForce(coordinates):
    ...
    M=[[int(np.around(math.sqrt(sum([(array_dots[i][d]-array_dots[j][d])**2 for d in[0,1]]))))for j in range(DOTS_N)]for i in range(DOTS_N)]
...
#     plt.show()
    ...
    ways_distance = [sum([int(np.around(math.sqrt(sum([(possibleWays[j][i+1][1][d]-possibleWays[j][i][1][d])**2 for d in[0,1]]))))for i in range(DOTS_N-1)]) + sum([int(np.around(math.sqrt(sum([(possibleWays[j][DOTS_N-1][1][d]-possibleWays[j][0][1][d])**2 for d in[0,1]]))))]) for j in range(len(possibleWays)-1)]
